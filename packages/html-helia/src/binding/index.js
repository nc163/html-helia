...
export const binding = async (element, blob, mediaType) => {
  switch (mediaType.type) {
    case 'text':
      await BindingText(element, blob, mediaType)
      break
    case 'image':
      await BindingImage(element, blob, mediaType)
      break
    case 'audio':
      await BindingAudio(element, blob, mediaType)
      break
    case 'video':
      await BindingVideo(element, blob, mediaType)
      break
    case 'application':
      await BindingApplication(element, blob, mediaType)
      break
    default:
